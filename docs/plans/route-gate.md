...
**Threat surface**: input: the request line itself, method and path, matched against a table fixed at wiring, nothing of an unmatched request being read, parsed or echoed back; authn/authz: this is the reachability boundary of a service, it decides what can be reached at all, while naming no caller and granting no right; a route named for GET is reached by HEAD too, the stdlib's matcher serving both from that handler, so a service opens HEAD without naming it, and what a refusal lists stays exactly what was named, never more; secrets: a refusal describes the request and never the table, the one exception being the methods listed for a path the caller already named; no egress, no dependency added.
